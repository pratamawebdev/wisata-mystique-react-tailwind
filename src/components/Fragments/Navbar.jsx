...
const Navbar = () => {
  ...
  return (
    <nav
      className={`navbar fixed top-0 left-0 z-50 w-full transition-all duration-400 md:backdrop-blur-xl md:backdrop-filter ${
        stickyNav
      ...
      <div className="container flex items-center justify-between h-24 navbar__container ">
        <Link
          ...
          className={` navbar__logo  text-5xl text-primaryColor ${
            stickyNav ? "text-gray-900" : "text-gray-800"
          }`}
        >
          Wisata Mystique
        </Link>
        ...
        <div
          className={`navbar__menu fixed top-0 z-10 flex h-full w-[70%] flex-col justify-between bg-white px-8 pt-24 pb-10 shadow-[0_-4px_12px_rgba(0,0,0,0.12)] transition-all duration-400 dark:bg-gray-900 md:static md:z-auto md:h-auto md:w-auto md:flex-row md:items-center md:gap-8 md:bg-transparent md:p-0 md:shadow-none md:dark:bg-transparent ${
            menuOpen ? "right-0" : "-right-full"
          }`}
        >
          <ul className="flex flex-col gap-8 mb-8 navbar__list md:mb-0 md:flex-row">
            {navItems.map(({ title, url }) => (
              <li key={url}>
                <Link
                  to={url}
                  className={`navbar__link group relative text-[15px] font-medium leading-tight text-black dark:text-white ${
                    stickyNav
                ...
                >
                  ...
                  <div
                    className={`absolute top-6 left-0 h-[3px] w-0 bg-blue-600  transition-all duration-400 group-hover:w-full  ${
                      stickyNav ? "md:bg-blue-600" : "md:bg-white"
                    }`}
                  />
                </Link>
              </li>
          ...
          <div
            className={`hidden cursor-pointer p-1 text-[1.3rem] dark:text-white md:flex ${
          ...
          {/* Tampilkan ProfileDropdown jika token ada */}
